report 54103 "Inventory Valuation List"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/InventoryValuationList.rdl';
    ApplicationArea = Basic, Suite;
    Caption = 'Envanter Basım Defteri - Stok Değerleme';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            column(StartDateText_; Format(StartDateText)) //included
            {
            }
            column(STRSUBSTNO_Text005_FORMAT_EndDate__; StrSubstNo(Text005, Format(EndDate)))
            {
            }
            column(FORMAT_EndDate__; Format(EndDate)) //included
            {
            }
